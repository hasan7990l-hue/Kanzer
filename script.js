  ...
  try {
    ...
    detailsData.items.forEach(function(item) {
      ...
      if (seconds >= 15 && seconds <= 60) {
        filteredVideos.push(videoObj);
      }
      backupVideos.push(videoObj);
    });

    if (filteredVideos.length === 0) {
      backupVideos.sort(function(a, b) { return a.duration - b.duration; });
      filteredVideos = backupVideos.slice(0, 6);
    } else {
      filteredVideos.sort(function(a, b) {
        return Math.abs(a.duration - 30) - Math.abs(b.duration - 30);
      });
      filteredVideos = filteredVideos.slice(0, 9);
    }
    ...
    filteredVideos.forEach(function(item) {
      var durationText = item.duration + ' ث';
      ...
      videoItem.addEventListener('click', function() {
        var allItems = document.querySelectorAll('#dashVideoResults > div > div');
        for (var x = 0; x < allItems.length; x++) {
          allItems[x].style.borderColor = 'rgba(0,102,255,0.3)';
          allItems[x].style.borderWidth = '2px';
        }
    ...
    hint.textContent = '💡 فيديوهات قصيرة (15-60 ثانية)';
